                            ...
                            <div className="flex flex-wrap justify-between items-start gap-3 mb-3">
                              <div className="flex items-start gap-3 flex-1 min-w-0">
                                ...
                                <div className="flex-1 min-w-0">
                                  <h5 className="font-bold text-gray-800 text-lg truncate" title={med.name}>{med.name}</h5>
                                  <div className="flex flex-wrap items-center gap-2 mt-0.5">
                                    <span className="text-xs text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded uppercase tracking-wider shrink-0">{med.dosage}</span>
                                    {med.isMaintenance && (
                                      <span className="text-[10px] text-green-600 font-bold bg-green-50 px-2 py-0.5 rounded-full border border-green-100 shrink-0">MAINTENANCE</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                              <span className={`px-4 py-1.5 text-[11px] font-black uppercase tracking-widest rounded-full shadow-sm shrink-0 ${med.status.toLowerCase() === 'taken' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>{med.status}</span>
                            </div>
